...
需求见 [`PERSONAL_LEARNING_OS.md`](PERSONAL_LEARNING_OS.md)（含学习机制）。一年计划见 [`LEARNING_PLAN.md`](LEARNING_PLAN.md)。本文件只指导工程顺序。**阶段 0 已完成。阶段 1 已在仓库落地。阶段 2 已落地。阶段 3 已完成。阶段 3b 已完成。阶段 4-1 已通过。阶段 4 已写入检索与固定工作流。阶段 4 整段尚未完成。** `main` 推送会触发 GitHub Pages；**未运行** `tools/publish.sh` 时自有服务器不会更新。不要把这两件事写成同一句「没有发布」。不要自动进入阶段 5。未明确要求时不要再次推送或发布。
...
**本轮（阶段 4-2 固定语料检索 + 检索—阅读—判定）：** 阶段 4-1 已通过。本轮写入核验助手第 2 周「在冻结语料里检索证据，对照两种方法」，以及第 3 周「按固定顺序做检索、阅读和判定」。默认当前练习仍是实验室 Attention。点周只更换练习说明，不把前面的周标成完成。第 2 周把检索与判定拆开评，检索命中不是结论正确。第 3 周沿用第 1 周准则，引用可定位，过期或冲突分开处理；不让模型自选下一步工具。不写 Agent 循环、MCP 或 FastAPI 服务作业。不把自写样本混进 CHEF 冒充官方分数。不拷贝 rag-service / Hello-Agents / mini-swe-agent 代码进主线。沿用已验收记录与复习规则。完成第 3 周不自动打开 Agent。不开始阶段 5。**阶段 4 整段尚未完成。** 第 0／24 章遗留修改仍单独处理。

**验证：** 核验助手出现第 2、3 周按钮；第 2 周指向第 28 章真实小节；第 3 周指向 28.8／28.10／23.17；两种检索对照与固定三步都是必需项；精排与服务化是拓展。默认首页仍是 Attention。实验室记录不受核验助手换周影响。

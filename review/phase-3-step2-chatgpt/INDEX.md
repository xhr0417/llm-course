...
- 审核入口：本文件
- 差异：`phase-3-step2.diff`（相对 `c0296a6`，不含第 0/24 章遗留修改）
...
`CourseLearning.isTaskComplete`：该任务全部 `required !== false` 的验收，**当前**结果必须是 `user_passed`。未检查、未通过都不算完成。来源固定为 `user_reported`，本轮不写 `auto`。

首页每条验收有三个选项：未检查 / 未通过 / 用户自报通过。控件用 `data-check`，HTML 不出现 `taskId`、`criterionId`、`lab.attention`。
...
修正后追加新结果，页面保留「曾经未通过的记录仍保留，完成只看当前结果。」刷新后 `llm-course-learning` 仍在。旧键 `llm-course-progress` 的已读不被改写；重置阅读进度不清除学习记录。
...
- 第一条先标「未通过」：仍停在 Attention，出现失败保留说明。再标「用户自报通过」：失败说明仍在，仍无跳转。
- 四条必需都标「用户自报通过」后出现「进入下一任务」，标题仍是 Attention。点按钮后才到第 2 周；`assembled-causal` 仍是未检查；「因果掩码 / 自己实现」仍勾着。
...
1. 未检查、未通过是否都不能让任务完成；是否必须全部必需项**当前**为用户自报通过。
2. 拓展未完成是否不阻挡；第 1 周概念记录是否不能自动完成第 2 周验收。
3. 修正后是否保留失败历史；刷新是否丢记录；旧阅读进度是否被改写或被当成掌握。

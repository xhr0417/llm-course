...
打开课程首页的 [我的学习](https://xhr0417.github.io/llm-course/#/)，默认练习是第 1 周 **小模型学习实验室—Attention**。同一页可以手动改到第 2–8 周：只更换练习说明，不表示前面已经完成。每一周都写清写什么、怎么检查、必需做到哪里。可以记下每条必需验收的当前结果（未检查／未通过／用户自报通过）；全部必需项当前自报通过后，由你确认才进入下一任务。本站不代写核心算法。

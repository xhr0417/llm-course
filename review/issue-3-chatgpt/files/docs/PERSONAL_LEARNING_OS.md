...
- **第一功能阶段只把这条边界写进约定和计划 JSON，不实现证据存储或表单。** 阶段 3 开工时先以这些规则为准，再按小步实现记录层。
...
卡点（阶段 3 记录层）：用户可写「拆头后不知道为何需要 transpose」；只影响首页补基础提示，不锁内容。
...
功能实施时改下列条目；未列出的视觉与无障碍约定全部保留。第 5 节四个独立维度与第 3.3 节证据来源／检查结果仍留给记录层，本阶段不宣称已经实现。

# 第 25 章 · Python Engineering for AI：从 0 亲手构建第一个真实项目
...
## 25.3 Learn 参考手册：工程基础（做项目时回来查）

以下内容是本 Step 用到的一切工程基础。先做项目，卡住时回来查对应小节——
比「先读完再动手」的学习效率高得多。
...
你在 Lab 里做的 log-analyzer 就是这个结构的最小实例：
`parser`（数据）→ `stats`（逻辑）→ `cli`（入口），三层互不 import 循环依赖。
...
指令微调数据（`projects/hf-mini-lab/data/sft_mini.jsonl`）长这样：
...
:::fold 练习：Step 5 的 bug 为什么值得用测试钉死？
```python
def test_best_step():
    stats = analyze(parse_log_file(SAMPLE))
    assert stats.best_step == 1000      # 未修复的实现会得到 550
```
我们的日志里除了 loss 行，还有 `WARNING step=550 val_loss increased`——它有 step 但没有 loss。
「所有 step」和「所有 loss」按下标对齐时，warning 行混进来就**错位**了。
修复：新增 `loss_steps` 只记录「同时带 loss 的 step」。
这就是你在 Step 5 亲手经历的修复。真实 bug 就是这样被测试逼出来的——不是靠盯代码。
:::

## 25.4 Level 3 · Reference Solution（完整工程实现）

:::unfold 目标
不复现「写个 CLI」的玩具，而是交付一个**真实项目**：训练日志分析器。跑完训练后（或训练崩了之后），它能回答：final loss、best loss、平均吞吐、warning 列表、是否出现 NaN。
:::

**项目位置**：[`projects/log-analyzer/`](https://github.com/xhr0417/llm-course/tree/main/projects/log-analyzer)（Guided Build 完成后对照；也可以直接跑）
...
tests/test_analyzer.py   # 14 个测试函数
```
...
| 知识点 | 项目中的位置 | 对应 Step |
| --- | --- | --- |
| 项目结构解耦 | parser / stats / cli 三层互不依赖 | Step 0-9 全程 |
| dataclass | `LogEvent`、`TrainRunStats` | Step 2 / 4 |
| typing | 全模块 type hints + `Optional` 指标 | Step 2 / 4 |
| pathlib | 文件输入一律 `Path`，不存在时退出码 2 | Step 1 / 7 |
| logging | `logger.info/error`，`--verbose` 控制级别 | Step 7 |
| argparse | `--json / --warnings / --verbose` | Step 6 |
| pytest | 14 个测试函数：解析 / 统计 / CLI / 子进程入口 | Step 1-8 |
| 异常处理 | 文件不存在 → 2；空日志 → 1；坏行跳过不崩 | Step 3 / 7 |
...
5. 你写过哪些 pytest？覆盖了哪些异常路径？Guided Build 里你自己的 3 个边界测试是什么？
6. 一个 HTTP 请求失败，你的重试策略是什么？为什么 4xx 不重试？
7. 10 个 LLM 请求如何做并发控制？`Semaphore` 解决什么问题？
8. CLI 工具的退出码有哪几类？为什么它对自动化很重要？
9. **（新版）** 描述 Step 5 的错位 bug：成因、症状、定位方法、修复方式。
10. **（新版）** 如果让你把这个项目扩展成「支持多份日志合并分析」，你会改哪一层？为什么？
...
| 站点 | 核心结论 |
| --- | --- |
| 学习模式 | Learn（参考手册）→ Guided Build（starter 10 步）→ Reference（完整工程） |
...
用于 | 第 26 章 HuggingFace Guided Build, Capstone 1（Eval Harness）, Capstone 2（RAG Service）
:::

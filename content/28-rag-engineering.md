...
真实结果：hybrid Recall@10 = **100%**（与 BM25 持平），MRR 0.8447。
注意 **hybrid 的 MRR 反而低于纯 BM25**（0.8447 < 0.8977）：向量结果把一部分原本排第一的正确文档挤到了后面。
...
| 模式 | MRR | nDCG@10 |
| --- | --- | --- |
| Hybrid | 0.8447 | 0.8843 |
| **Hybrid + Reranker** | **0.9318** | **0.9497** |

精排把 MRR 从 0.845 提到 **0.932**——整条管线里收益最大的一步。
:::
...
| 指标 | 公式直觉 | 敏感于 |
| --- | --- | --- |
| Recall@k | top-k 里**有没有**至少一条相关 | 召回能力 |
| MRR | 第一条相关的排名倒数 | 第一个就命中的能力 |
| nDCG@k | 相关结果按位置折扣累加 | 整体排序质量 |

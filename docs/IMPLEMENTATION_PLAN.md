...
**本轮（阶段 3b-4 失败后重练入口，暂不收尾）：** 3b-2、3b-3 均已通过。本轮只给失败记下的薄弱知识点提供重练入口：指向当前练习，或同一主线上更早、包含该知识点的更小任务。匹配同时看任务的 `conceptIds` 和 `criteria[].conceptId`（第 2 周拓展「归一化与位置」关联 `modern-block`，不在本周 `conceptIds` 里）。点了才会更换练习说明；不会按日历打开新的主线任务，也不会打开更晚的周。不删除或改写原实现/验证记录，不把历史 `user_reported` 改成失败，不改首页当前任务除非使用者点了更小任务。拓展仍不阻挡任务完成。不做自动评分、LLM judge、SM-2、题库、streak、dashboard、通知。不开始阶段 4。**3b-4 暂不收尾。3b 整段尚未收尾。** 第 0／24 章遗留修改仍单独处理。

...
需求见 [`PERSONAL_LEARNING_OS.md`](PERSONAL_LEARNING_OS.md)（含学习机制）。一年计划见 [`LEARNING_PLAN.md`](LEARNING_PLAN.md)。本文件只指导工程顺序。**阶段 0 已完成。阶段 1 已在仓库落地。阶段 2 已落地。阶段 3 已完成。阶段 3b 已完成。阶段 4-1 已通过。阶段 4-2 已通过。阶段 4 整段尚未完成。** `main` 推送会触发 GitHub Pages；**未运行** `tools/publish.sh` 时自有服务器不会更新。不要把这两件事写成同一句「没有发布」。不要自动进入 Agent 或阶段 5。未明确要求时不要再次推送或发布。
...
**本步实际完成（阶段 3b-4 失败后重练入口，已通过）：** 3b-2、3b-3 均已通过。失败记下的薄弱知识点给出重练入口：指向当前练习，或同一主线上更早、包含该知识点的更小任务。匹配同时看任务的 `conceptIds` 和 `criteria[].conceptId`（第 2 周拓展「归一化与位置」关联 `modern-block`，不在本周 `conceptIds` 里）。点了才会更换练习说明；不会按日历打开新的主线任务，也不会打开更晚的周。不删除或改写原实现/验证记录，不把历史 `user_reported` 改成失败，不改首页当前任务除非使用者点了更小任务。拓展仍不阻挡任务完成。对照 [`fb2dacf`](https://github.com/xhr0417/llm-course/commit/fb2dacf)，补丁 CI [35833043998](https://github.com/xhr0417/llm-course/actions/runs/35833043998) success，审核包 CI [35833104797](https://github.com/xhr0417/llm-course/actions/runs/35833104797) success。不开始阶段 4。第 0／24 章遗留修改仍单独处理。

**完成条件：** 3b-1 已封板；3b-2、3b-3、3b-4 已通过。机制层 C 的到期队列、通过／失败记录与失败后重练入口已落地。终审未独立重跑 375／768／1280，不阻挡收尾。**阶段 3b 已完成。** 完成后停止，不进入阶段 4。
...
**本步实际完成（阶段 4-1 手动切阶段 + 第一条判定任务，已通过）：** 阶段 3b 已完成。首页可手动切到「技术声明核验助手 / 单 Agent」；该阶段给出第一条可执行练习「给定声明和证据，只做判定」。默认当前练习仍是实验室 Attention，不按日历打开核验助手。点阶段只更换练习说明，不把实验室标成完成。换周限定在当前阶段。保存核验助手验收不改实验室旧依据、失败历史和概念记录。不会自动跨阶段推进。本周不做检索、不写 Agent 循环、不用 LLM 裁判当金标准、不把自写样本混进 CHEF 冒充官方分数。92 项与四个机制参考仓库仍只在文档中出现。沿用已验收记录与复习规则。对照 [`521dfcd`](https://github.com/xhr0417/llm-course/commit/521dfcd)，实现 CI [35836180046](https://github.com/xhr0417/llm-course/actions/runs/35836180046) success，审核包 CI [35836268274](https://github.com/xhr0417/llm-course/actions/runs/35836268274) success。终审未独立重跑浏览器和 375／768／1280，不阻挡通过。不开始阶段 5。**阶段 4 整段尚未完成。** 第 0／24 章遗留修改仍单独处理。

**验证：** 主线当前阶段可手动切到核验助手；默认首页仍是 Attention；核验助手第一条练习指向真实教材小节；92 项与四个机制参考仓库仍只在文档中出现。未核验的 Atlas 机制继续按本站设计，不引入其代码。

**本步实际完成（阶段 4-2 固定语料检索 + 检索—阅读—判定，已通过）：** 阶段 4-1 已通过。本轮写入核验助手第 2 周「在冻结语料里检索证据，对照两种方法」，以及第 3 周「按固定顺序做检索、阅读和判定」。默认当前练习仍是实验室 Attention。点周只更换练习说明，不把前面的周标成完成。第 2 周把检索与判定拆开评，检索命中不是结论正确。第 3 周沿用第 1 周准则，引用可定位，过期或冲突分开处理；不让模型自选下一步工具。不写 Agent 循环、MCP 或 FastAPI 服务作业。不把自写样本混进 CHEF 冒充官方分数。不拷贝 rag-service / Hello-Agents / mini-swe-agent 代码进主线。沿用已验收记录与复习规则。完成第 3 周不自动打开 Agent。对照 [`13a7a70`](https://github.com/xhr0417/llm-course/commit/13a7a70)，实现 CI [35838814639](https://github.com/xhr0417/llm-course/actions/runs/35838814639) success，审核包 CI [35838894532](https://github.com/xhr0417/llm-course/actions/runs/35838894532) success。终审未独立重跑浏览器和 375／768／1280，不阻挡通过。不开始阶段 5。**阶段 4 整段尚未完成。** 第 0／24 章遗留修改仍单独处理。

**验证：** 核验助手出现第 2、3 周按钮；切周不会误入实验室。第 2 周指向第 28 章真实小节；第 3 周指向 28.8／28.10／23.17；两种检索对照与固定三步都是必需项；精排与服务化是拓展。第 3 周全部必需项通过后仍没有「进入下一任务」。保存核验助手记录不改实验室旧依据和失败历史。默认首页仍是 Attention。

**进入 Agent 前需拍板：** 第 3 周轨迹目前按练习记录验收（手写 jsonl 即可）。与 Agent 比较时需要一份可重复运行的工作流基线。4-2 不把可运行脚本列为完成门槛。两者都可用于当前学习。不开始 Agent。

**本步（4-2）不做：** 一次写完第 3–6 月全部周任务；Agent 循环；MCP；FastAPI 服务作业。后续仍按「先判定、再检索、再 Agent」加入 Agent。

**本阶段仍不做：** 92 项选修库 UI；拷贝 Hello-Agents / mini-swe-agent / CHEF / rag-service 代码进主线；阶段 5。
